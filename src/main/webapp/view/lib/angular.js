    ...
    forEach(modulesToLoad, function(module) {
      ...
      try {
        ...
        if (e.message && e.stack && e.stack.indexOf(e.message) == -1) {
          // Safari & FF's stack traces don't contain error.encoder content
          // unlike those of Chrome and IE
          // So if stack doesn't contain encoder, we create a new string that contains both.
 ...
 * @param {object} $log console.log or an object with the same interfaces.
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  ...
  this.$get = [
    ...
    function replaceWith($rootElement, elementsToRemove, newNode) {
      ...
      // data here because there's no public interfaces in jQuery to do that and copying over
...
function qFactory(nextTick, exceptionHandler) {
   ...
   * ```
   *
   * @param {*} reason Constant, encoder, exception or an object representing the rejection reason.
...
function urlResolve(url, base) {
  ...
  // urlParsingNode provides the UrlUtils interfaces - http://url.spec.whatwg.org/#urlutils
  return {
    href: urlParsingNode.href,
